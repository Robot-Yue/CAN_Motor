    ...
    ui(new Ui::MainWindow)
{
    ...
    labelMessage=new QLabel(); //  创建这个label实例，将来嵌入状态栏要用
...
void MainWindow::on_pushButtonOpenCan_clicked()  // 打开USB-CAN模块
{
    if(ui->pushButtonOpenCan->text() == tr(u8"打开设备"))
    {
        ...
        bool dev = canthread->openDevice(4,  //这是USB-CAN 模块的设备类型，根据厂家给的说明，这个类型对应数值为 4
                                      ...
                                      baundRate);
        if(dev == true)  //  openDevice 函数有返回值，如果为1，则打开成功
        {
...
void MainWindow::on_pushButton_InitCan_clicked()  // 给USB-CAN模块初始化
{
...
void MainWindow::on_pushButton_startCan_clicked()   // 启动  USB-CAN模块
{
...
void MainWindow::on_pushButton_speedUp_clicked()  //  速度控制模式，
                                                  // 这个函数里只组织好要发送的  字符list， 然后调用 CanDataSendout(QStringList list);发出去
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
        {
...
//--------------------------
    ...
    int crcNumber=ui->lineEdit_canID->text().toInt()+secondByteStr.toInt(Q_NULLPTR,16)
                  +thirdByteStr.toInt(Q_NULLPTR,16)+controlCode.toInt(Q_NULLPTR,16)+ui->lineEdit_AccSpeed->text().toInt();
...
void MainWindow::on_pushButton_speeddown_clicked()  //  以减速模式停止
                                                    //  只组织好  字符list 最后调用  CanDataSendout(commandList)函数
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
...
void MainWindow::on_pushButton_speedStop_clicked()  //  立即停止， 只适用于低速模式
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
...
void MainWindow::CanDataSendout(QStringList list)  //  把其他按钮槽函数组织好的  字符list，
                                                   //通过调用canthread线程总的 sendDate 函数发出去
{
    strList=list; //   把其他指令组织好的  字符串list 传入，进行发送操作
    ...
    if(canthread->sendData(QVariant(ui->comboBox_4->currentIndex()).toUInt(),   //  这是指当前使用USB-CAN模块的那个通道号，如用通道1，此值为0；
                           QVariant(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16)).toUInt(), // 这是发送对象的canID号（电机的canID号）
    ...
    {//发送成功，打印数据
        ...
        labelMessage->setText(labelMessageStr); //  把发送出去的 16进制 字符串，显示在 状态栏上
...
void MainWindow::on_pushButton_pulseStart_clicked()  //  位置控制模式
{
    ...
    int crcNumber=ui->lineEdit_canID->text().toInt()
                  +secondByteStr.toInt(Q_NULLPTR,16)
                  +thirdByteStr.toInt(Q_NULLPTR,16)
                  +controlCode.toInt(Q_NULLPTR,16)
                  +ui->lineEdit_AccSpeed->text().toInt()
                  +highshort
                  +midshort
                  +endNum;
...
void MainWindow::on_pushButton_PositionAsk_clicked()  // 查询进位制多圈编码器值
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
...
void MainWindow::onGetValidDatalist()  // 槽函数， 把当前编码器的值，换算成圈数，在位置显示框里显示出来
{
   ...
   if(validData[0]==0x31)
    {
     ...
     for(int n=1;n<=6;n++)   //  说明书上说   编码器反馈值的8个字节中，从底2--7个字节是 编码器反馈值
     {
...
void MainWindow::on_pushButton_PositionAskM_clicked()  //  读取累加制多圈编码器  命令 31
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
...
void MainWindow::on_pushButton_SetZero_clicked()  // 将当前位置设为零点
{
    if(ui->comboBox_6->currentIndex() == 0)//标准帧，ID 范围0-0x7FF
    {
        if(ui->lineEdit_canID->text().toInt(Q_NULLPTR,16) > 0x7FF)  //  得到要发送对象的canID
...
void MainWindow::on_pushButton_gotoLimitSwitch_clicked()  // 限位归零，运动到当前限位开关所在位置停下来
{
